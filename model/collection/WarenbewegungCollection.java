...
public class WarenbewegungCollection<Warenbewegung> extends ArrayList {
     ...
     * @throws SQLException Nutzt als Ausgangslage eine Liste aller
     * Warenbestands-Objekte, führt für die Filterung für jedes
     * WarenbestandFilterModel-Attribut eine Schleife aus. Die
     * Zwischenspeicherung führt über ein Listen - Ping - Pong statt. Gibt die
     * gefilterte Collection zurück.
     */
    public WarenbewegungCollection<Warenbewegung> applyFilter(WarenbewegungFilterModel wfm) throws SQLException{
        
        Dao<model.Warenbewegung, Integer> warenbewegungDao = DatabaseManager.getInstance().getWarenbewegungDao();
        List<model.Warenbewegung> resultA = new ArrayList();
        List<model.Warenbewegung> resultB = warenbewegungDao.queryForAll();
        ...
        if (resultB.size() > 0) {
            ...
            if (wfm.getExpireID() > 0) {
                for (int i = 0; i < resultB.size(); i++) {
                    if (resultB.get(i).getHaltbarkeitsDatum() != null) {
                ...
                for (int i = 0; i < resultA.size(); i++) {
                    if (resultA.get(i).getHaltbarkeitsDatum().after((wfm.getHaltbarVon()))
                            && resultA.get(i).getHaltbarkeitsDatum().before(wfm.getHaltbarBis())){
            ...
            for (int i = 0; i < resultA.size(); i++) {
                ...
                for (ZielPosition zp : fc) {
                    if ((zp.getLagerfach().getLager().getLagerort().equals(wfm.getzLagerort())
                            ...
                            && !resultB.contains(resultA.get(i))) {
                        resultB.add(resultA.get(i));
                    }
                }
            }
            ...
            for (int i = 0; i < resultB.size(); i++) {
                ...
                for (ZielPosition zp : fc) {
                    if (((wfm.getzX() == 0)
                            ...
                            && !resultA.contains(resultB.get(i))){
                        resultA.add(resultB.get(i));
                    }
                }
            }
